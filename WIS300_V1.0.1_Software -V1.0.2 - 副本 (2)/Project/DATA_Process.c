...
unsigned char FrameDataReceived=0;
...
///////////////////////////////////////////////////////////////////////////////////////////////		
				...
				{
					if(Ch_A_ID & FrameDataReceived){Return_SampleData(Ch_A_ID,Sample_Control.Ch_DATASize);while(DMASendDataCompleted);DMASendDataCompleted=1;}//向TPA发送数据					
					if(Ch_B_ID & FrameDataReceived){Return_SampleData(Ch_B_ID,Sample_Control.Ch_DATASize);while(DMASendDataCompleted);DMASendDataCompleted=1;}//向TPA发送数据
					if(Ch_C_ID & FrameDataReceived){Return_SampleData(Ch_C_ID,Sample_Control.Ch_DATASize);while(DMASendDataCompleted);DMASendDataCompleted=1;}//向TPA发送数据
					if(Ch_D_ID & FrameDataReceived){Return_SampleData(Ch_D_ID,Sample_Control.Ch_DATASize);while(DMASendDataCompleted);DMASendDataCompleted=1;}//向TPA发送数据	
					GPIO_ResetBits(GPIOB,GPIO_Pin_3);Send_Flag_TPA=0;Clear_Flag_TPA &=0x00;
					FrameDataReceived=0;
				}
