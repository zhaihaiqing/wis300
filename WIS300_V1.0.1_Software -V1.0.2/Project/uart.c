...
void UART1_DMA_SendData(unsigned char *BufAddress,unsigned int Length)
{
	...
	tmpreg |= DMA_DIR_PeripheralDST | DMA_Mode_Normal |                //DMA的转换模式为DST模式，由内存搬移到外设  |   //正常模式
            DMA_PeripheralInc_Disable | DMA_MemoryInc_Enable |		   //接收一次数据后，设备地址禁止后移          |   //内存地址自增
            DMA_PeripheralDataSize_Byte|DMA_PeripheralDataSize_Byte |  //DMA搬移数据尺寸，Byte就是为8位            |   //     
            DMA_Priority_VeryHigh | DMA_M2M_Disable;				   //                                          |   //
	
	DMA1_Channel4->CCR = tmpreg;
	DMA1_Channel4->CPAR = (uint32_t)(&USART1->DR);//外设地址
	DMA1_Channel4->CMAR = (uint32_t)BufAddress;//缓存地址
	DMA1_Channel4->CNDTR = Length;//DMA缓存大小，当发送N个字节的数据后自动产生中断
